...
function createItemRow(item) {
    ...
    row.innerHTML = `
        ...
        <td data-label="Ações" class="actions-cell">
            <div class="actions-container">
                <button class="btn btn-sm btn-warning" data-action="${ACTIONS.ITEM_EXIT}" data-id="${item.id}" title="Saída / Empréstimo"><i class="fas fa-sign-out-alt"></i></button>
    ...
    row.querySelector('.actions-dropdown-content').addEventListener('click', (event) => {
        ...
        if (link) {
            const dropdown = link.closest('.actions-dropdown-content');
            if (dropdown) {
                dropdown.classList.add('hidden');
                if (dropdown.classList.contains('fixed-dropdown')) {
                    dropdown.classList.remove('fixed-dropdown');
...
function renderItemsTable(filteredItems) {
    ...
    const massAddButton = document.querySelector('[data-action="mass-add"]');
    ...
    if (allItems.filter(i => i.type !== 'Kit').length === 0) {
        ...
        `;
        if (massAddButton) massAddButton.style.display = 'none';
        return;
    }

    if (massAddButton) massAddButton.style.display = 'inline-flex';
    ...
    if (typeFilter.value === "") {
        typeFilter.classList.add('placeholder-selected');
    } else {
        typeFilter.classList.remove('placeholder-selected');
    }
    ...
    renderPaginatedTable({
        ...
        data: filteredItems,
        renderRowFunction: createItemRow,
        noDataMessage: searchTerm ? `Nenhum item encontrado para "${searchTerm}".` : 'Nenhum item para o tipo selecionado.',
        tableType: 'item'
    });
}
...
function renderCollaboratorsTable(collaborators) {
    ...
    const massAddButton = document.querySelector('[data-action="mass-add-collaborator"]');
...
function openAdjustmentModal(itemId) {
    ...
    openModal(MODAL_IDS.ADJUSTMENT, (modal) => {
        ...
        modal.querySelector('#adjustment-system-stock').textContent = item.totalStock;
        form.elements.physicalCount.value = item.totalStock;
...
function renderPriceHistoryContent(priceHistory, modal) {
    ...
    if (!priceHistory || priceHistory.length === 0) {
        container.innerHTML = '<p>Nenhum histórico de preço registrado.</p>';
    } else {
        const rows = [...priceHistory]
            .sort((a, b) => new Date(b.date) - new Date(a.date))
            .map(record => `<tr><td>${new Date(record.date).toLocaleDateString('pt-BR')}</td><td>R$ ${record.price.toFixed(2)}</td></tr>`)
            .join('');
        container.innerHTML = `<div class="table-responsive"><table class="item-table"><thead><tr><th>Data da Atualização</th><th>Preço de Custo</th></tr></thead><tbody>${rows}</tbody></table></div>`;
    }
}
...
function renderServiceOrderItemsTable(os, modal) {
    ...
    if (osItems.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center;">Nenhum item alocado para esta O.S.</td></tr>';
        return;
    }
    ...
    osItems.forEach(osItem => {
        ...
        row.innerHTML = `
            ...
            <td class="actions-cell">
                <button class="btn btn-sm btn-danger" data-action="${ACTIONS.REMOVE_ITEM_FROM_OS}" data-os-id="${os.id}" data-alloc-id="${osItem.allocationId}" title="Remover Item da O.S.">
                    <i class="fas fa-trash-alt"></i>
                </button>
            </td>
        `;
...
function openServiceOrderDetailsModal(osId) {
    ...
    openModal(MODAL_IDS.SERVICE_ORDER_DETAILS, (modal) => {
        ...
        const totalCost = (os.items || []).reduce((sum, osItem) => {
            const itemData = allItems.find(i => i.id === osItem.itemId);
    ...
    openModal(MODAL_IDS.KIT_RETURN, (modal) => {
        ...
        detailsView.addEventListener('input', (event) => {
            if (event.target.name === 'loss_quantity') {
                ...
                document.getElementById(`debit-subtotal-${componentId}`).textContent = `R$ ${subtotal.toFixed(2)}`;
                updateTotalDebit();
            }
        });
